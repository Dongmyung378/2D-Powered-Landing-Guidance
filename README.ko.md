...
현재 저장소에는 검증된 시뮬레이션 환경, 두 가지 비학습 수직 착륙 기준선, 수평 위치·자세 직렬 제어기, 통합 PID 착륙 제어기, 재현 가능한 기준선 튜닝과 분리 외란 평가가 구현되어 있습니다. 최적제어, Behavior Cloning, DAgger, 복합 불확실성과 대규모 Monte Carlo 평가는 이후 로드맵에서 진행합니다.
